    ...
    <header class="Header">
      <div class="container">
        ...
        <nav class="Header__Menu">
          ...
          <nuxt-link class="Header__Link" :to="$i18n.path('team')" exact>
            {{ $t('links.about') }}
          </nuxt-link>
          <nuxt-link class="Header__Link" v-if="$i18n.locale === 'en'" :to="`/sc` + $route.fullPath" active-class="none" exact>
            {{ $t('links.french') }}
          </nuxt-link>
          <nuxt-link class="Header__Link" v-else :to="$route.fullPath.replace(/^\/[^\/]+/, '')" active-class="none" exact>
            {{ $t('links.english') }}
          </nuxt-link>
          <button @click="test">萨达所</button>
        </nav>
      </div>
    </header>
    <nuxt/>
